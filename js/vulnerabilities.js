...
class VulnerabilitiesManager {
    ...
    parseExcelDate(excelDate) {
        ...
        // Fix the time format if it has hyphens instead of colons (YYYY-MM-DD HH-MM-SS -> YYYY-MM-DD HH:MM:SS)
        if (dateStr.match(/^\d{4}-\d{2}-\d{2} \d{2}-\d{2}-\d{2}$/)) {
            dateStr = dateStr.replace(/^(\d{4}-\d{2}-\d{2}) (\d{2})-(\d{2})-(\d{2})$/, '$1 $2:$3:$4');
        }
...
// Wrapper functions for inline HTML event handlers
window.handleXlsxImportSafe = function(event) {
    console.log('[VULN] handleXlsxImportSafe called');
    if (window.vulnerabilitiesManager) {
        window.vulnerabilitiesManager.handleXlsxImport(event);
    } else {
        console.error('Vulnerabilities manager not initialized');
